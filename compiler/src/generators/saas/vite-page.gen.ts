...
function generatePageCode(schema: FluskSchema, primary: string): string {
  ...
  if (isAuth) code += `  const { data = {}, isLoading } = ${hookName}();\n`;
...
export class VitePageGenerator {
  async generate(schemas: FluskSchema[], outputDir: string): Promise<GenerationResult> {
    ...
    try {
      ...
      const hookBarrelExports: string[] = [];
      ...
      for (const view of views) {
        try {
          ...
          if ((view as any).auth === true) {
            const hookName = getHookName(view);
            await writeFileFs(join(outputDir, 'hooks', `${hookName}.ts`), hookStubTemplate(hookName));
            result.filesGenerated += 1;
            hookBarrelExports.push(`export { ${hookName} } from './${hookName}';`);
          }
        } catch (err) {
      ...
      await writeFileFs(join(outputDir, 'hooks', 'index.ts'), `${HEADER}\n\n${hookBarrelExports.join('\n')}\n`);
      result.filesGenerated += 1;
